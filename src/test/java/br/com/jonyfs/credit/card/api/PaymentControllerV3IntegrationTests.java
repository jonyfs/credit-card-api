...
public class PaymentControllerV3IntegrationTests {
	...
	public void setUp() throws Exception {
		...
		mockMvc = MockMvcBuilders.webAppContextSetup(wac)
				.apply(documentationConfiguration(this.restDocumentation).snippets()).build();
		MockitoAnnotations.initMocks(this);
	...
	public void verifyIfPaymentHasNoData() throws Exception {
		...
		mockMvc.perform(post(ResourcePaths.Payment.V3.ROOT).contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(status().isUnprocessableEntity()).andExpect((jsonPath("$", notNullValue())))
				.andExpect((jsonPath("$.code", notNullValue()))).andExpect((jsonPath("$.message", notNullValue())))
				...
				.andDo(document("." + ResourcePaths.Payment.V3.ROOT + "/{method-name}",
						...
						responseFields(
								fieldWithPath("code").type(JsonFieldType.STRING)
										.description("The code id of problem. Use it for support only"),
								fieldWithPath("exception").type(JsonFieldType.STRING).description("Exception"),
								fieldWithPath("message").type(JsonFieldType.STRING)
										.description("The message to explain the problem"),
								fieldWithPath("errors").type(JsonFieldType.ARRAY)
										.description("Array with constraint problems"))))
				.andReturn();
	}
		...
		mockMvc.perform(post(ResourcePaths.Payment.V3.ROOT).contentType(MediaType.APPLICATION_JSON)
				...
				.andDo(document("." + ResourcePaths.Payment.V3.ROOT + "/{method-name}",
						...
						requestFields(fieldWithPath("id").type(JsonFieldType.STRING)
								.description("The Credit Card Transaction ID."),
								fieldWithPath("cardType").type(JsonFieldType.STRING).description("Credit Card Type")
										.attributes(key("constraints").value(NotNull.class.getSimpleName())),
								fieldWithPath("cardNumber").type(JsonFieldType.STRING).description("Credit Card Number")
								...
								fieldWithPath("expirationDate").type(JsonFieldType.OBJECT)
										...
										.attributes(key("constraints").value(NotNull.class.getSimpleName())),
								fieldWithPath("store").type(JsonFieldType.OBJECT).description("Store")
										.type(JsonFieldType.ARRAY)
										.attributes(key("constraints").value(NotNull.class.getSimpleName())),
								fieldWithPath("products").type(JsonFieldType.ARRAY).description("Products Array")
										.type(JsonFieldType.ARRAY)
